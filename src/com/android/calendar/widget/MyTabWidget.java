...
public class MyTabWidget extends LinearLayout implements OnFocusChangeListener {
	...
	private void initTabWidget() {
		...
		/*
		 * try { int par = 0x400; Method method =
		 * this.getClass().getSuperclass()
		 * .getSuperclass().getDeclaredMethod("setBooleanFlag", new
		 * Class[]{int.class,boolean.class}); method.setAccessible(true);
		 * method.invoke(this, par,true); } catch (SecurityException e) {
		...
		if (context.getApplicationInfo().targetSdkVersion <= Build.VERSION_CODES.DONUT) {
			...
			mBottomLeftStrip = resources
					.getDrawable(R.drawable.tab_bottom_left_v4);
			mBottomRightStrip = resources
					.getDrawable(R.drawable.tab_bottom_right_v4);
		} else {
			// Use modern color scheme for Eclair and beyond
			mBottomLeftStrip = resources
					.getDrawable(R.drawable.tab_bottom_left);
			mBottomRightStrip = resources
					.getDrawable(R.drawable.tab_bottom_right);
		}
	...
	public void dispatchDraw(Canvas canvas) {
		...
		/*
		 * if (!mDrawBottomStrips) { // Skip drawing the bottom strips. return;
		 * }
		 * 
		 ...
		 * 
		 * if (mStripMoved) { Rect selBounds = new Rect(); // Bounds of the
		 * selected tab indicator selBounds.left = selectedChild.getLeft();
		 * selBounds.right = selectedChild.getRight(); final int myHeight =
		 * getHeight(); mBottomLeftStrip.setBounds( Math.min(0, selBounds.left -
		 * mBottomLeftStrip.getIntrinsicWidth()), myHeight -
		 * mBottomLeftStrip.getIntrinsicHeight(), selBounds.left, getHeight());
		 * mBottomRightStrip.setBounds( selBounds.right, myHeight -
		 * mBottomRightStrip.getIntrinsicHeight(), Math.max(getWidth(),
		 * selBounds.right + mBottomRightStrip.getIntrinsicWidth()), myHeight);
		 * mStripMoved = false; }
		 * 
		 * mBottomLeftStrip.draw(canvas); mBottomRightStrip.draw(canvas);
		 */
	}
