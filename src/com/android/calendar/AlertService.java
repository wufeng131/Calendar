...
public class AlertService extends Service {
    ...
    private static AlertService mService;
    ...
    void processMessage(Message msg) {
     ...
     boolean updateAlertNotification(Context context) {
    ...
    private  void postNotification(Context context, SharedPreferences prefs,
...
//            if (vibrateAlways || (vibrateSilent && nowSilent)) {
            ...
            if(!TextUtils.isEmpty(reminderRingtone) && !checkIsInCall()) {
...
//for bug 15768
    public boolean  checkIsInCall() {
        if(TelephonyManager.getPhoneCount()>1){
            ...
            int state1 = TelephonyManager.getDefault(0).getCallState();
            int state2 = TelephonyManager.getDefault(1).getCallState();
            if(state1 != TelephonyManager.CALL_STATE_IDLE && state2 != TelephonyManager.CALL_STATE_IDLE && !isVTCall){
                return true;
            }else{
                return false;
            }
            ...
            if(state != TelephonyManager.CALL_STATE_IDLE && !isVTCall){
                return true;
            }else{
                return false;
            }
