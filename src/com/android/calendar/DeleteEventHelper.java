...
public class DeleteEventHelper {
    ...
    private DialogInterface.OnClickListener mDeleteNormalDialogListener =
        ...
        public void onClick(DialogInterface dialog, int button) {
            ...
            //Modify start for bug 17928
            updateAlertNotification();
            //Modify end for bug 17928
            if (mExitWhenDone) {
                mParent.finish();
            }
        }
    ...
    private void updateAlertNotification() {
        ...
        try {
            if (cursor != null && !cursor.moveToFirst()) {
                ...
                AlertService.getInstance().updateAlertNotification(mParent.getBaseContext());
            }
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    public void deleteAllEvents() {
        Cursor cursor = mParent.managedQuery(Calendar.Events.CONTENT_URI, EVENT_PROJECTION, null, null, null);
        if (cursor.getCount() == 0) {
            Toast.makeText(mParent, mParent.getResources().getString(R.string.delete_error), Toast.LENGTH_LONG).show();
            return;
        }
        mContentResolver.delete(Calendar.Events.CONTENT_URI, null /* where */, null /* selectionArgs */);
        updateAlertNotification();
    }
}
