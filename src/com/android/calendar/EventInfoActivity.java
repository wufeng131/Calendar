...
public class EventInfoActivity extends Activity implements View.OnClickListener,
    ...
    private static final int MENU_GROUP_SEND = 4;
    ...
    protected void onCreate(Bundle icicle) {
        ...
        String where = String.format(Locale.US,CALENDARS_WHERE, mEventCursor.getLong(EVENT_INDEX_CALENDAR_ID));
        mCalendarsCursor = getContentResolver().query(uri, CALENDARS_PROJECTION, where, null, null);
        ...
        if(mCalendarOwnerAccount == null || TextUtils.isEmpty(mCalendarOwnerAccount)){
            mCalendarOwnerAccount = "";
        }
        String eventOrganizer = mEventCursor.getString(EVENT_INDEX_ORGANIZER);
        if(!mCalendarOwnerAccount.equalsIgnoreCase(eventOrganizer)){
            mIsOrganizer = true;
        }else{
        ...
        where = String.format(Locale.US,ATTENDEES_WHERE, mEventId);
        mAttendeesCursor = getContentResolver().query(uri, ATTENDEES_PROJECTION, where, null,
        ...
        if (hasAlarm) {
            ...
            where = String.format(Locale.US,REMINDERS_WHERE, mEventId);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        boolean hasBlueTooth = SystemProperties.getBoolean("ro.device.support.bt", true);
        if(hasBlueTooth){
          item = menu.add(MENU_GROUP_SEND,MENU_SEND, 0, R.string.send);
 ...
 private void doSend()
  {
    Vcalendar nVcalendar = new Vcalendar(this);

    VcalendarInfo Calendarsinfo = new VcalendarInfo();
    Cursor cursor = mEventCursor;
    if(cursor==null) return ;
    Calendarsinfo.eventitle = cursor.getString(EVENT_INDEX_TITLE);
    Calendarsinfo.timezone = cursor.getString(EVENT_INDEX_EVENT_TIMEZONE);
    Calendarsinfo.allDay = cursor.getInt(EVENT_INDEX_ALL_DAY) != 0;

    Calendarsinfo.location = mEventCursor.getString(EVENT_INDEX_EVENT_LOCATION);
    Calendarsinfo.description = mEventCursor.getString(EVENT_INDEX_DESCRIPTION);
    Calendarsinfo.rRule = mEventCursor.getString(EVENT_INDEX_RRULE);
    Calendarsinfo.hasAlarm = mEventCursor.getInt(EVENT_INDEX_HAS_ALARM) != 0;
    if (Calendarsinfo.hasAlarm) {
        Uri uri = Reminders.CONTENT_URI;
        String where = String.format(Locale.US,REMINDERS_WHERE, mEventId);
        Cursor reminderCursor = getContentResolver().query(uri, REMINDERS_PROJECTION, where, null,
                REMINDERS_SORT);
    ...
    Log.i(TAG, "doSend:Calendarsinfo.timezone "+Calendarsinfo.timezone);

    Calendarsinfo.starttime = mStartMillis;
    Calendarsinfo.endtime = mEndMillis ;

    Log.i(TAG, "doSend:"+Calendarsinfo);

    String sdStatus = Environment.getExternalStorageState();
    if(!sdStatus.equals(Environment.MEDIA_MOUNTED)) {
        Log.d(TAG, "SD card is not avaiable/writeable right now.");
        Toast.makeText(this,"SD card is not avaiable/writeable right now.",
                Toast.LENGTH_SHORT).show();
     return;
    }
    String filename = nVcalendar.VcalendarCreate(Calendarsinfo);
    ...
    File localFile = new File("mnt/sdcard/bluetooth/"+filename);

    Uri localUri = Uri.fromFile(localFile);
    Log.i(TAG, "doSend:"+localUri);
    intent.putExtra("android.intent.extra.STREAM", localUri);
    intent.setClassName("com.android.bluetooth", "com.android.bluetooth.opp.BluetoothOppLauncherActivity");
    //intent.setDataAndType(localUri, "text/x-vcalendar");
    startActivity(intent);
  }
...
//                    }
                    textView.setOnTouchListener(new OnTouchListener() {
                        public boolean onTouch(View v, MotionEvent event) {
                            try {
                                ...
                                // ignore
                                Log.v(TAG,"ActivityNotFoundException "+e);
                                return true;
                            }catch(Exception e){
    ...
    protected void onDestroy() {
        ...
        if (mAttendeesCursor != null && !mAttendeesCursor.isClosed()) {
        	mAttendeesCursor.close();
        	mAttendeesCursor = null;
        }
